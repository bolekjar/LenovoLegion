...
std::string LenovoUSBController::getName()
{
    return name;
}

std::string LenovoUSBController::getLocation()
{
    return location;
}

void LenovoUSBController::setZoneLeds(uint8_t zone_num, std::vector<std::pair<uint8_t, RGBColor>> &led_colors)
{
...
std::vector<uint8_t> LenovoUSBController::getInformation(uint8_t information_id)
{
    ...
    if(num_bytes > 0)
    {
        std::vector<uint8_t> response(&read_buffer[0], &read_buffer[num_bytes]);
        return response;
    }
