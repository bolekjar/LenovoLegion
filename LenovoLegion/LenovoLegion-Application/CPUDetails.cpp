...
CPUDetails::CPUDetails(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::CPUDetails)
{
    ui->setupUi(this);
}
...
void CPUDetails::init(const legion::messages::CPUInfo &data,const legion::messages::CPUTopology &topology,int freqMin,int freqMax)
{
...
void CPUDetails::closeEvent(QCloseEvent *event)
{
    emit closed();
    hide();
    event->ignore();
